...
{
    public class GastronomyCeremonyCategoryParser
    {
        public static TagLinked ParseLTSGastronomyCeremonyCategory(
            ...
            )
        {
            ...
            try
            {
                var ltsdata = ltsresult.ToObject<LTSData<LTSGastronomyCeremonyCodeData>>();
                dataid = ltsdata.data.rid;

                return ParseLTSGastronomyCeremonyCategory(ltsdata.data, reduced);
            }
            catch(Exception ex)
            {
                //Generate Log Response on Error
                Console.WriteLine(JsonConvert.SerializeObject(new { operation = "gastronomy.ceremonycode.parse", id = dataid, source = "lts", success = false, error = true, exception = ex.Message }));
        ...
        public static TagLinked ParseLTSGastronomyCeremonyCategory(
            LTSGastronomyCeremonyCodeData data, 
            bool reduced)
        {        
