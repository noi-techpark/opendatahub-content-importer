...
{
    public class EventTagParser
    {
        public static TagLinked ParseLTSEventTag(
            ...
            )
        {
            ...
            try
            {
                ...
                return ParseLTSEventTag(ltsdata.data, reduced);
            }
            catch(Exception ex)
            {
                //Generate Log Response on Error
                Console.WriteLine(JsonConvert.SerializeObject(new { operation = "event.tag.parse", id = dataid, source = "lts", success = false, error = true, exception = ex.Message }));
